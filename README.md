...
**▶ 바로 써보기: https://daker-finance-ai-challenge-2026.vercel.app/** — 설치도 로그인도 없습니다.
...
- 답변은 실제 공식 자료를 근거로 삼고 출처를 밝힙니다 — 찾기쉬운 생활법령정보(법제처)·보건복지부·금융감독원·서민금융진흥원에서 정리한 33건
...
처음 열면 **가상 인물 '김새봄'의 데이터로 화면이 이미 채워져 있습니다.** 아무것도 입력하지 않아도
준비도·브리핑·현금흐름·스캐너까지 전부 그대로 눌러볼 수 있습니다. 상단에 예시를 보고 있다는 띠가 계속 떠 있고,
이 데이터는 저장되지 않습니다.

- **내 정보로 시작하기** — 입력 패널이 열립니다. 내 값을 넣으면 화면이 그 값으로 다시 계산됩니다.
- **예시 데이터로 저장** — 김새봄의 데이터를 그대로 저장해 예시 띠 없이 둘러봅니다.
...
│  ├─ verify/page.tsx       신뢰성 검증 (퍼징을 열 때마다 서버에서 재계산)
│  └─ api/
│     ├─ chat/              대화 (실시간 스트리밍 + 카드)
...
│     └─ rag/reindex/       근거 문서 임베딩 색인 (설치할 때 한 번)
├─ components/
...
│  ├─ ProblemIntro.tsx      첫 화면 — 왜 이 앱이 필요한가 (공공 통계)
│  ├─ ProfilePanel.tsx      내 정보 입력 패널
│  └─ TrustBadge.tsx        이 숫자를 누가 만들었나 배지 (앱이 계산 / AI 해석 / 공식 자료)
└─ lib/
   ├─ forecast.ts           잔액 예측 계산 (AI 아님)
   ├─ readiness.ts          자립 준비도 점수 계산 (AI 아님)
   ├─ budget.ts             목돈 배분 규칙 (AI 아님)
   ├─ simulator.ts          배분 슬라이더 계산 & 진단 (AI 아님)
   ├─ briefing.ts           규칙 기반 브리핑 (AI 실패 시 대체)
   ├─ llm.ts                AI 호출 (스트리밍·구조화 출력·이미지·모델 전환)
   ├─ schema.ts             AI 응답 형식 정의 (Gemini·Claude 공용)
   ├─ prompt.ts             시스템 프롬프트 & 카드 도구 설명
   ├─ toolCards.ts          도구 호출 → 카드 (AI가 넘긴 금액은 버림)
   ├─ cards.ts              카드 타입 & 검증
   ├─ rag.ts                임베딩 & pgvector 검색
   ├─ grounding.ts          검색 결과를 프롬프트 근거 블록으로
   ├─ profile.ts            프로필 모델 & 샘플 인물
   ├─ profileStore.ts       내 정보 저장 (브라우저 localStorage)
   ├─ stats.ts              화면에 쓰는 공공 통계의 단일 출처
   ├─ verification.ts       환각 차단 퍼징 (/verify 1번)
   ├─ adversarialResult.ts  적대적 프롬프트 실측 기록 (/verify 2번)
   ├─ fraudEval.ts          사기 평가셋 19건 (라벨 포함, 공개)
   ├─ fraudEvalResult.ts    사기 탐지 실측 기록 (/verify 3번)
   └─ serverCache.ts        같은 상황이면 AI 응답 재사용 (무료 한도 절약)
```
...
| 검증 | 방법 | 결과 |
| --- | --- | --- |
| 1. 구조적으로 막히는가 | 악의적 인자(5억원·음수·NaN·배열 주입) × 상황별 프로필 조합을 전부 투입 | **144/144 차단** |
| 2. 실제 AI도 뚫지 못하는가 | 금액을 지어내도록 유도하는 프롬프트를 실 API에 투입 | **불일치 0건** |
| 3. 사기를 실제로 잡아내는가 | 금감원 공개 수법 기반 평가셋 19건 | **정확도 100% · 미탐 0 · 오탐 0** |

1번 퍼징은 `/verify`를 열 때마다 서버에서 다시 계산합니다 (미리 적어둔 숫자가 아닙니다).
2·3번은 실제 API로 측정한 기록이며 `src/lib/adversarialResult.ts`, `src/lib/fraudEvalResult.ts`에 남아 있습니다.
검증 코드는 `src/lib/verification.ts`, 사기 평가셋 원문은 `src/lib/fraudEval.ts`에 그대로 공개돼 있습니다.
